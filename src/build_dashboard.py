...
<main class="main">
  <header class="topbar">
    ...
    <div class="top-actions">
      <input id="llm-key" class="apikey" type="password" placeholder="API key OpenAI · solo Agente + LLM" autocomplete="off">
...
async function lanzarBrief(id, modo){
  ...
  if(modo==='llm' && !claveUI()){
    pintarBrief(r, {
      vista:'pyme', src:'sin-clave', situacion:'El LLM no se ha llamado. Pega una API key de OpenAI en la barra de arriba (se queda en este navegador) y pulsa otra vez Agente + LLM.',
    ...
    toast('Pega la API key arriba para el LLM');
    return;
  }
  ...
  if(modo!=='fondo'){
    try{
      ...
      if(res.ok){
        ...
        if(modo==='llm' && j.fuente!=='agente') toast(j.aviso||'El LLM no se usó');
