...
variables escala-libre (ratios, meses de runway, porcentajes). Un umbral de
"flujo > 20.000" puntúa a una pyme de 50k/mes y a un grupo de 5M/mes con la
misma vara, lo cual es incorrecto.
...
def score_mes(row):
    crudos = {
        ...
        "eficiencia": f_eficiencia(row.get("burn_rate_3m_avg"),
        ...
        "tendencia": f_tendencia(row.get("flujo_pendiente_6m"),
                                 row.get("recuperacion_prov"),
                                 row.get("recuperacion_clientes")),
        "apalancamiento": f_apalancamiento(row.get("deuda_sobre_ingresos"),
                                           bool(row.get("caja_negativa_flag", 0))),
    }

    # Un factor no aplicable no se imputa a 50: se redistribuye su peso entre
    # los que sí tienen dato. Así la ausencia de información no premia ni castiga.
    activos = {k: v for k, v in crudos.items() if v[0] is not None}
    if not activos:
        return pd.Series({"score_mensual": np.nan, "peso_cubierto": 0.0, "detalle": {}})
    ...
    return pd.Series({"score_mensual": round(float(score), 2),
...
def agregar(hist):
    """Media exponencial con más peso en los meses recientes."""
    h = hist.dropna(subset=["score_mensual"])
    if h.empty:
        return np.nan
    n = len(h)
    w = np.exp(-np.log(2) * (n - 1 - np.arange(n)) / cfg.HALF_LIFE_MONTHS)
...
def run():
    ...
    for cid, hist in panel.groupby("company_id"):
        ...
        fila = {
            ...
            "confianza": hist["confianza"].iloc[-1] if "confianza" in hist else "n/d",
    ...
    cols = ["company_id", "score_final", "clasificacion", "tendencia", "confianza"]
    print("\n=== TOP 5 SALUDABLES ===")
